...
let turnModelLightOn = false
// Hier die code die de ontvanger uit moet voeren.
function recieverCode() {
    if (turnModelLightOn == false) {
        turnModelLightOn = true
    } else {
        turnModelLightOn = false
    }
    // Hier de code die de ontvanger uitvoert.
}

loops.forever(function () {
    if (input.switchRight() && turnModelLightOn) {
        if (turnModelLightOn && recievedPotentiometer >= 140 && currentBrightness >= 100 && currentBrightness < recievedPotentiometer) {
            currentBrightness = currentBrightness + 5
        }
        if (turnModelLightOn && recievedPotentiometer >= 140 && currentBrightness <= 100 && currentBrightness < recievedPotentiometer) {
            currentBrightness = currentBrightness + 10
        }
        if (turnModelLightOn && recievedPotentiometer >= 140 && currentBrightness > recievedPotentiometer) {
            currentBrightness = currentBrightness - 5
        }
        if (turnModelLightOn && recievedPotentiometer < 140 && currentBrightness >= 0) {
            currentBrightness = currentBrightness - 10
        }
        if (!turnModelLightOn) {
        ...
        light.setAll(color.rgb(currentBrightness - 100, currentBrightness - 100, currentBrightness - 100))
        console.log(currentBrightness - 100)
    }
})
...
network.onInfraredReceivedNumber(function (num) {
    if (input.switchRight()) {
        if (num >= 100) {
            recievedPotentiometer = num
            console.log(`recievedPotentiometer aangepast naar ${recievedPotentiometer}`)
        }
    }
...
// Potentiometer dimt licht of muziek, stuurt in elk geval een waarde via infrarood
loops.forever(function () {
    if (input.switchRight() == false) {
        potentiometer = Math.round((crickit.signal4.analogRead() / 3.95) - 3)
        network.infraredSendNumber(potentiometer + 100)
